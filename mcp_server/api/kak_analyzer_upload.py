# mcp_server/api/kak_analyzer_upload.py
...
from mcp_server.utils.ingestion_manifest_utils import save_status, build_kak_unique_key
from mcp_server.utils.kak_path_utils import save_kak_pdf
...
def _background_ingest_sync(
    filename: str, pelanggan: str, project: str, tahun: str, job_id: str
):
    """
    Wrapper sinkron agar cocok dengan Starlette BackgroundTasks.
    Menjalankan coroutine async `process_ingest_kak_file` di event loop terpisah.
    """
    import anyio

    anyio.run(process_ingest_kak_file, filename, pelanggan, project, tahun, job_id)


# ====================
# Upload endpoint
# ====================
@router.post("/upload-kak/")
async def upload_kak(
    ...
    if getattr(kak_tools, "_manifest", None) and unique_key in kak_tools._manifest:
        save_status(
            job_id,
            "skipped",
    ...
    save_status(
        job_id,
        "pending",
        "File tersimpan. Menunggu ingestion KAK/TOR",
        result={"unique_key": unique_key},
    )
...
async def process_ingest_kak_file(
    filename: str, pelanggan: str, project: str, tahun: str, job_id: str
):
    ...
    try:
        # 2) ingestion proses
        ingest_result = await kak_tools.ingest_kak_file(
            filename=filename,
            pelanggan=pelanggan,
            project=project,
            tahun=tahun,
        ...
        if ingest_result.get("status") != "success":
            raise RuntimeError(
                ingest_result.get("message", "KAK tersimpan tapi gagal ingest.")
            )
        ...
        save_status(
            job_id,
            "running",
            "KAK berhasil diingest ke knowledge base.",
            result=ingest_result,
        )

        # 3) summary analysis
        summary = await kak_tools.generate_kak_summarize(
            filename=filename,
            pelanggan=pelanggan,
            project=project,
            tahun=tahun,
        ...
        msg = f"Gagal proses ingestion KAK: {e}"
